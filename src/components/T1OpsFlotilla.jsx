  ...
  return (
    <div style={{ background: C.panelGrad, borderRadius: 12, padding: "20px 22px", border: `1px solid ${C.border}`, flex: 1, minWidth: 0 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        ...
        <div style={{ width: 36, height: 36, borderRadius: 8, backgroundColor: color + "26", display: "flex", alignItems: "center", justifyContent: "center", color }}>
          {icon}
        </div>
      </div>
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;font-family:'Segoe UI', Arial, sans-serif}
  ...
  // Tablas separadas: solo última milla y solo media milla, por proveedor.
  const umPorProveedor = carrierCostList.filter(c => c.costoUM > 0).sort((a, b) => b.costoUM - a.costoUM);
  const mmPorProveedor = carrierCostList.filter(c => c.costoHM > 0).sort((a, b) => b.costoHM - a.costoHM);
  ...
  return (
    <div>
      ...
      {/* 1) Costo Última Milla por proveedor */}
      {umPorProveedor.length > 0 && (
        <div style={{ backgroundColor: C.white, borderRadius: 12, border: "1px solid " + C.border, overflow: "hidden", marginBottom: 20 }}>
          <div style={{ padding: "14px 18px", borderBottom: "1px solid " + C.border, fontSize: 13, fontWeight: 700, color: C.text }}>
            Costo Última Milla por Proveedor
            <span style={{ fontWeight: 500, color: C.green, fontSize: 11, marginLeft: 8 }}>(solo última milla)</span>
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "2px solid " + C.border }}>
                {["Proveedor", "Rutas", "Costo Última Milla"].map(h => (
                  <th key={h} style={{ padding: "8px 14px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C.textMuted, textTransform: "uppercase" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {umPorProveedor.map((c, i) => (
                <tr key={i} style={{ borderBottom: "1px solid " + C.border }}>
                  <td style={{ padding: "10px 14px", fontSize: 13, fontWeight: 600 }}>{c.carrier}</td>
                  <td style={{ padding: "10px 14px", fontSize: 13 }}>{c.rutasUM}</td>
      ...
      {/* 2) Costo Media Milla por proveedor */}
      {mmPorProveedor.length > 0 && (
        <div style={{ backgroundColor: C.white, borderRadius: 12, border: "1px solid " + C.border, overflow: "hidden", marginBottom: 20 }}>
          <div style={{ padding: "14px 18px", borderBottom: "1px solid " + C.border, fontSize: 13, fontWeight: 700, color: C.text }}>
            Costo Media Milla por Proveedor
            <span style={{ fontWeight: 500, color: C.blue, fontSize: 11, marginLeft: 8 }}>(solo media milla / half mile)</span>
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "2px solid " + C.border }}>
                {["Proveedor", "Rutas", "Costo Media Milla"].map(h => (
                  <th key={h} style={{ padding: "8px 14px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C.textMuted, textTransform: "uppercase" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {mmPorProveedor.map((c, i) => (
                <tr key={i} style={{ borderBottom: "1px solid " + C.border }}>
                  <td style={{ padding: "10px 14px", fontSize: 13, fontWeight: 600 }}>{c.carrier}</td>
                  <td style={{ padding: "10px 14px", fontSize: 13 }}>{c.rutasHM}</td>
      ...
      {/* 3) Costo General por proveedor (Total = ÚM + MM) */}
      {umMmPorProveedor.length > 0 && (
        <div style={{ backgroundColor: C.white, borderRadius: 12, border: "1px solid " + C.border, overflow: "hidden", marginBottom: 20 }}>
          <div style={{ padding: "14px 18px", borderBottom: "1px solid " + C.border, fontSize: 13, fontWeight: 700, color: C.text }}>
            Costo General por Proveedor
            <span style={{ fontWeight: 500, color: C.textMuted, fontSize: 11, marginLeft: 8 }}>(última milla + media milla)</span>
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "2px solid " + C.border }}>
      ...
      <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        <div style="text-align:center;margin-bottom:16px;">
          <svg width="38" height="36" viewBox="0 0 42 40" fill="none" xmlns="http://www.w3.org/2000/svg" style="height:36px;"><path d="M27.7336 16.8035H31.7201C31.8555 16.8035 31.9847 16.8408 32.0954 16.9091C32.0954 16.9277 32.0954 16.9463 32.0954 16.9711V36.9153C32.0954 37.7843 32.9321 38.3988 33.7503 38.1443C33.7626 38.1443 33.7749 38.1381 33.7872 38.1319C35.6451 37.387 38.0813 36.4001 39.8408 35.5993C40.1607 35.469 40.5483 35.2889 40.5668 34.6992V16.9711C40.5668 16.9339 40.5668 16.9028 40.5668 16.8718V16.8097V4.662C40.5668 3.94816 39.9885 3.36467 39.281 3.36467H36.2972C35.762 3.36467 35.276 3.69987 35.0914 4.20887C33.8672 7.6291 31.554 10.1803 28.0535 10.8135C27.8935 10.8445 27.7336 10.8693 27.5675 10.8879C26.9277 10.9686 26.4478 11.5211 26.4478 12.1729V15.5062C26.4478 16.22 27.0261 16.8035 27.7336 16.8035Z" fill="#D93A26"/><path d="M32.9999 2.03011C32.8276 1.8563 32.6 1.74457 32.3416 1.74457H3.03329C2.11049 1.74457 1.36609 2.42738 1.36609 3.27157V7.74705C1.36609 8.82713 1.56295 9.30509 2.9041 9.30509H12.692C12.9504 9.30509 13.1596 9.51614 13.1596 9.77685V13.5323C13.1596 13.5509 13.1596 13.5633 13.1596 13.5757V34.6186C13.1596 35.0841 13.4733 35.4565 13.8978 35.6428C14.7714 36.0214 17.749 37.4988 18.7456 38.0264C20.0129 38.703 21.5263 37.6167 21.5263 36.1704V10.4907C21.5263 10.4286 21.5202 10.3045 21.5202 10.3045C21.4464 9.96927 21.5632 9.57821 21.8955 9.44165C21.9693 9.41062 22.0616 9.39199 22.1538 9.39199H25.3591C25.3714 9.39199 25.3775 9.39199 25.3775 9.39199C32.3539 8.70298 33.0737 3.26537 33.1291 2.65084C33.1291 2.6136 33.1291 2.58256 33.1291 2.55152V2.40255C33.1291 2.26599 33.0799 2.12943 32.9876 2.03011H32.9999Z" fill="#D93A26"/></svg>
...
export default function T1OpsFlotilla({ user, onLogout }) {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "var(--font-inter), -apple-system, BlinkMacSystemFont, sans-serif", backgroundColor: C.bg, overflow: "hidden" }}>
      {/* Sidebar */}
      <aside style={{
        ...
        <div style={{ padding: sidebarCollapsed ? "16px 12px" : "16px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "center", minHeight: 58 }}>
          <img src="/logo-t1.svg" alt="T1 Envíos" style={{ height: sidebarCollapsed ? 24 : 32, objectFit: "contain" }} />
        </div>
